...
#                embed.set_thumbnail(url="https://www.beehive.systems/hubfs/Icon%20Packs/Orange/alert-circle-outline.png")
            ...
            if last_seen != 'N/A':
                embed.add_field(name="Last check in", value=f"**`{last_seen}`** seconds ago", inline=False)
            
            last_seen_pos = aircraft_data.get('seen_pos', 'N/A')
            if last_seen_pos != 'N/A':
                embed.add_field(name="Last position update", value=f"**`{last_seen_pos}`** seconds ago", inline=False)
