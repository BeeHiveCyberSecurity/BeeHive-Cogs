    ...
    async def _send_aircraft_info(self, ctx, response):
        if 'ac' in response and response['ac']:
            ...
            if aircraft_model is not None:
                embed.add_field(name="Aircraft model", value=f"`{aircraft_model}`", inline=True)
    ...
    async def stats(self, ctx):
        ...
        try:
            ...
            if all(key in data for key in ["beast", "mlat", "other", "aircraft"]):
                ...
                embed2.add_field(name="Law enforcement aircraft indexed", value="{:,} aircraft".format(len(self.law_enforcement_icao_set)), inline=False)
                embed2.add_field(name="Military aircraft indexed", value="{:,} aircraft".format(len(self.military_icao_set)), inline=False)
                embed2.add_field(name="Medical aircraft indexed", value="{:,} aircraft".format(len(self.medical_icao_set)), inline=False)
