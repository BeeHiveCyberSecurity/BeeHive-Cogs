...
#                embed.set_thumbnail(url="https://www.beehive.systems/hubfs/Icon%20Packs/Orange/alert-circle-outline.png")
            if squawk_code == '7500':
                embed = discord.Embed(title='Aircraft information', description=f"# {aircraft_data.get('desc', 'N/A')}", color=discord.Colour(0xFF9145))
            ...
            view.add_item(discord.ui.Button(label=f"Track {icao} live", url=f"{link}", style=discord.ButtonStyle.link))
    ...
    async def stats(self, ctx):
        ...
        try:
            ...
            if all(key in data for key in ["beast", "mlat", "other", "aircraft"]):
                ...
                embed2.add_field(name="Damaged aircraft", value="{:,} known".format(len(self.global_prior_known_accident_set)), inline=False)
                embed2.add_field(name="Wartime aircraft", value="{:,} observed".format(len(self.ukr_conflict_set)), inline=False)
                embed2.add_field(name="Utility aircraft", value="{:,} spotted".format(len(self.agri_utility_set)), inline=False)
