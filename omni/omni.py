...
class Omni(commands.Cog):
    ...
    async def check_monitoring_reminder(self, message):
        ...
        # Check if the message count has reached 50 or if it's been more than 10 minutes since the last reminder
        last_reminder_time = await self.config.guild(guild).last_reminder_time()
        current_time = datetime.utcnow()
        if (self.memory_user_message_counts[guild.id][channel.id] >= 50 or 
