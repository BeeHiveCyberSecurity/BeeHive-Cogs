...
class Omni(commands.Cog):
    ...
    async def process_message(self, message):
        try:
            ...
            self.increment_statistic('global', 'global_statistics.message_count')
            ...
            if message.attachments:
                for attachment in message.attachments:
                    if attachment.content_type.startswith("image/") and not attachment.content_type.endswith("gif"):
                        ...
                        self.increment_statistic('global', 'global_statistics.image_count')
    ...
    def update_moderation_stats(self, guild_id, message, text_category_scores):
        ...
        self.increment_statistic('global', 'global_statistics.moderated_count')
        ...
        try:
            ...
            if timeout_duration > 0:
                try:
                    ...
                    self.increment_statistic('global', 'global_statistics.timeout_count')
                    self.increment_statistic(guild.id, 'total_timeout_duration', timeout_duration)
                    self.increment_statistic('global', 'global_statistics.total_timeout_duration', timeout_duration)
                ...
                for guild_id, stats in self.memory_stats.items():
                    if guild_id == 'global':
                        for stat_name, value in stats.items():
                            current_value = await self.config.get_attr(f"global_statistics.{stat_name}")()
                            await self.config.get_attr(f"global_statistics.{stat_name}").set(current_value + value)
                    else:
                        guild_conf = self.config.guild_from_id(guild_id)
                        for stat_name, value in stats.items():
                            current_value = await guild_conf.statistics.get_attr(stat_name)()
                            await guild_conf.statistics.get_attr(stat_name).set(current_value + value)

                for guild_id, user_counts in self.memory_user_message_counts.items():
                    if guild_id != 'global':
                        guild_conf = self.config.guild_from_id(guild_id)
                        current_user_counts = await guild_conf.statistics.user_message_counts()
                        for user_id, count in user_counts.items():
                            current_user_counts[user_id] = current_user_counts.get(user_id, 0) + count
                        await guild_conf.statistics.user_message_counts.set(current_user_counts)

                for guild_id, users in self.memory_moderated_users.items():
                    if guild_id == 'global':
                        current_users = await self.config.global_statistics.moderated_users()
                        for user_id, count in users.items():
                            current_users[user_id] = current_users.get(user_id, 0) + count
                        await self.config.global_statistics.moderated_users.set(current_users)
                    else:
                        guild_conf = self.config.guild_from_id(guild_id)
                        current_users = await guild_conf.statistics.moderated_users()
                        for user_id, count in users.items():
                            current_users[user_id] = current_users.get(user_id, 0) + count
                        await guild_conf.statistics.moderated_users.set(current_users)

                for guild_id, counter in self.memory_category_counter.items():
                    if guild_id == 'global':
                        current_counter = Counter(await self.config.global_statistics.category_counter())
                        current_counter.update(counter)
                        await self.config.global_statistics.category_counter.set(dict(current_counter))
                    else:
                        guild_conf = self.config.guild_from_id(guild_id)
                        current_counter = Counter(await guild_conf.statistics.category_counter())
                        current_counter.update(counter)
                        await guild_conf.statistics.category_counter.set(dict(current_counter))
