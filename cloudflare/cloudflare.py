...
class Cloudflare(commands.Cog):
    ...
    @commands.is_owner()
    @r2.command()
    async def search(self, ctx, bucket_name: str, file_name: str):
        """Search for a file in the specified R2 bucket."""
        api_tokens = await self.bot.get_shared_api_tokens("cloudflare")
        api_key = api_tokens.get("api_key")
        email = api_tokens.get("email")
        bearer_token = api_tokens.get("bearer_token")
            ...
            async with self.session.get(url, headers=headers) as response:
                ...
                if not matching_files:
                    embed = discord.Embed(
                        ...
                        description=f"No file named `{file_name}` found in bucket `{bucket_name}`.",
                        color=0xff4545
                    )
                else:
                    embed = discord.Embed(
                        title="File found",
                        description=f"File `{file_name}` found in bucket `{bucket_name}`.",
                        color=discord.Color.green()
                    )
                    embed.add_field(
                        name="File details",
                        value=f"**`{matching_files[0]}`**",
                        inline=False
                    )
                await ctx.send(embed=embed)
        except RuntimeError as e:
